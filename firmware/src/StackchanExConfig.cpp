...
void StackchanExConfig::setExtendSettings(DynamicJsonDocument doc)
{
    ...
    _ex_parameters.openclaw.host    = doc["openclaw"]["host"].as<String>();
    _ex_parameters.openclaw.port    = doc["openclaw"]["port"].as<int>();
    _ex_parameters.openclaw.model   = doc["openclaw"]["model"].as<String>();
...
void StackchanExConfig::printExtParameters(void)
{
    ...
    M5_LOGI("openclaw host: %s", _ex_parameters.openclaw.host.c_str());
    M5_LOGI("openclaw port: %d", _ex_parameters.openclaw.port);
    M5_LOGI("openclaw model: %s", _ex_parameters.openclaw.model.c_str());
